...
async function loadYouTubeAdminData() {
  ...
  if (ce || ae) {
    body.innerHTML = `<tr><td colspan="10">${(ce || ae).message}</td></tr>`;
    return;
  }
  ...
  (customers || []).forEach(c => {
    ...
    const tr=document.createElement("tr");
    tr.innerHTML=`<td>${c.full_name||"-"}</td><td>${c.email||"-"}</td><td>${a?.channel_name||c.channel_name||"-"}</td><td>${a?.google_connected?"Connected ✅":"Not Connected"}</td><td>${Number(a?.subscribers||0).toLocaleString("en-IN")}</td><td>${Number(a?.views||0).toLocaleString("en-IN")}</td><td>${Number(a?.videos||0).toLocaleString("en-IN")}</td><td>${a?.monetization_status||"pending"}</td><td>${a?.manager_access ? "GRANTED ✅" : "PENDING 🟡"}</td><td>${a?.google_connected ? `<a class="btn primary" href="manage-channel.html?customer=${encodeURIComponent(c.id)}">${a?.manager_access ? "Manage Channel" : "Access Setup"}</a>` : "-"}</td>`;
    body.appendChild(tr);
  });
  if (!(customers||[]).length) body.innerHTML='<tr><td colspan="10">No customers yet.</td></tr>';
}
