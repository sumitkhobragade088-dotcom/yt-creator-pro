...
async function loadCustomers(){
  ...
  customers=(data||[]).sort((a,b)=>String(a.full_name||a.name||a.email||a.id||"").localeCompare(String(b.full_name||b.name||b.email||b.id||""));
  const sel=$("freeServiceCustomer"); if(!sel)return;
  sel.innerHTML='<option value="">Select customer…</option>'+customers.map(c=>`<option value="${esc(c.id)}">${esc(c.full_name||c.name||c.email||c.id)}${c.email?` • ${esc(c.email)}`:""}</option>`).join("");
}
...
async function loadServices(){
  const {data,error}=await supabase.from("service_charges").select("*");
  if(error){msg(`Service list load failed: ${error.message}`);return;}
  services=(data||[]).filter(s=>s.is_active!==false).sort((a,b)=>(Number(a.sort_order??999999)-Number(b.sort_order??999999))||String(a.service_name||"").localeCompare(String(b.service_name||""));
}
function fillServices(){
  const ssel=$("freeServiceType"),csel=$("freeServiceChannel"),btn=$("grantFreeService");
  if(!ssel)return;
  ssel.innerHTML=services.map(s=>`<option value="${esc(s.service_name||s.name||"")}">${esc(s.service_name||s.name||"Service")} — FREE</option>`).join("");
