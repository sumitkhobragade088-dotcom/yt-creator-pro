...
function syncServiceUI(){
  ...
  if(toggle) toggle.textContent=values.size ? `${values.size} service${values.size>1?"s":""} selected` : "Select services…";
...
async function loadChannels(customerId){
  ...
  ssel.disabled=true; renderServiceOptions(); syncServiceUI();
}
...
function renderServiceOptions(){
  const sel=$("freeServiceType"),box=$("freeServiceOptions");
  if(!sel||!box)return;
  ...
  if($("freeServiceChannel")?.value){
    sel.disabled=false;
    box.innerHTML=services.map(s=>`<label class="yt-service-dd-item">
      <input type="checkbox" data-service-value value="${esc(s.service_name)}" ${previous.has(s.service_name)?"checked":""}>
      <span class="svc-text"><span class="svc-name">${esc(s.service_name)}</span><span class="svc-desc">${esc(s.description||"Service")}</span></span>
      ${s.charge!=null?`<span class="svc-price">₹${Number(s.charge).toLocaleString("en-IN")}</span>`:""}
    </label>`).join("");
    box.querySelectorAll('input[data-service-value]').forEach(i=>i.addEventListener("change",()=>{
      const o=[...sel.options].find(x=>x.value===i.value);if(o)o.selected=i.checked;syncServiceUI();
    }));
  }else{
    sel.disabled=true;box.innerHTML="<small>Select customer and channel first…</small>";
  }
  syncServiceUI();
}
...
async function grant(){
  ...
  [...ssel.options].forEach(o=>o.selected=false); renderServiceOptions();
}
...
function bind(){
  ...
  $("freeServiceDropdownToggle")?.addEventListener("click",()=>$("freeServiceDropdown")?.classList.toggle("open"));
  document.addEventListener("click",e=>{const dd=$("freeServiceDropdown");if(dd&&!dd.contains(e.target))dd.classList.remove("open");});
