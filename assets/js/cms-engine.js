...
const DEFAULTS={
  ...
  website_cms:{nav:{},hidden:[],order:[],sections:{},cards:{},buttons:{},customPages:[],customButtons:[],customSections:[],customSidebar:[],brand:{name:'',tagline:''},footer:'',media:{logoUrl:'',heroImageUrl:''},contact:{hidden:false,kicker:'CONTACT & SUPPORT',title:'How Can We Help?',subtitle:'Contact YT Creator Pro for service, payment or creator-support questions.',email:'sumitkhobragade088@gmail.com',phone:'9637832490',whatsapp:'919637832490',hours:'24 X 7',helpText:'Tell us what you need help with. We will use your configured email application to send the message.',safetyNote:'Never share your password, OTP, recovery code or other sensitive Google/YouTube account information.'}},
...
function normalizeSetting(key,value){
  if(key!=='website_cms') return value;
  const out=value||{};
  out.contact={...(out.contact||{})};
  const d=DEFAULTS.website_cms.contact;
...
async function currentAdminUser(requiredPermission='website_cms.manage'){
  const {data:sessionData,error:sessionError}=await withTimeout(supabase.auth.getSession(),CMS_TIMEOUT,'CMS authorization');
...
async function applyAdmin(){
  ...
  for(const [id,v] of Object.entries(c.buttons||{})){const el=document.getElementById(id);if(!el)continue;if(v.label)el.textContent=v.label;el.hidden=!!(v.hidden||v.inactive||v.deleted||v.status==='draft');if(v.href&&el.tagName==='A')el.href=v.href;if(el.tagName==='A'&&v.newTab!==undefined){el.target=v.newTab?'_blank':'_self';if(v.newTab)el.rel='noopener noreferrer'}}
  for(const [key,v] of Object.entries(c.blocks||{})){const el=document.querySelector(`[data-cms-block-key="${CSS.escape(key)}"]`);if(el){el.hidden=!!(v.hidden||v.inactive||v.deleted||v.status==='draft');if(v.order!==undefined)el.style.order=String(v.order)}}
  for(const x of (c.customButtons||[]).filter(cmsLive).sort((a,b)=>(a.order??999)-(b.order??999))){if(document.querySelector(`[data-cms-admin-btn="${x.id}"]`))continue;const host=document.querySelector(x.host||'.yt-premium-header-actions')||document.querySelector('.yt-premium-header-actions');if(!host)continue;const a=document.createElement(x.href?'a':'button');a.className='yt-premium-top-btn';a.dataset.cmsAdminBtn=x.id;a.textContent=x.label||'New Button';a.style.order=String(x.order??999);if(x.href){a.href=x.href;a.target=x.newTab?'_blank':'_self';if(x.newTab)a.rel='noopener noreferrer'}host.appendChild(a);applyCmsVisibility(a,x)}
...
function siteNavConfig(c,key){const legacyHidden=(c.hidden||[]).includes(key),v=c.nav?.[key];const out=typeof v==='string'?{label:v}:{label:'',...(v||{})};out.hidden=!!(legacyHidden||out.hidden||out.inactive||out.deleted||out.status==='draft');return out}

function applyCmsVisibility(el,v){if(!el||!v)return;const page=currentPage();const ctx=location.pathname.includes('/admin/')?'admin':page==='dashboard.html'?'user':['login.html','register.html','google-callback.html'].includes(page)?'auth':'public';const a=v.audience||'all';if(a!=='all'&&a!==ctx)el.hidden=true;if(v.visibility){el.dataset.cmsDesktop=v.visibility.desktop===false?'hide':'show';el.dataset.cmsMobile=v.visibility.mobile===false?'hide':'show'}}

function renderVirtualPage(c,slug){
  const x=(c.customPages||[]).find(v=>v.slug===slug&&cmsLive(v));if(!x)return false;
...
async function applyWebsite(){
  ...
  if(page==='index.html'||page===''||page==='page.html'){
    ...
    const nav=document.querySelector('.yt-user-public-nav');if(nav){const links=[...nav.querySelectorAll('a:not([data-cms-site-btn])')];links.forEach(a=>{const k=a.getAttribute('href'),v=siteNavConfig(c,k);if(v.label)a.textContent=v.label;a.hidden=!!v.hidden;if(v.href)a.href=v.href});sortByOrder(links,c.order||[],a=>a.getAttribute('href'));for(const x of (c.customButtons||[]).filter(v=>cmsLive(v)&&(!v.placement||v.placement==='nav')).sort((a,b)=>(a.order??999)-(b.order??999))){if(nav.querySelector(`[data-cms-site-btn="${x.id}"]`))continue;const a=document.createElement('a');a.dataset.cmsSiteBtn=x.id;a.href=x.href||'#';a.textContent=x.label||'New';a.style.order=String(x.order??999);if(x.newTab){a.target='_blank';a.rel='noopener noreferrer'}nav.appendChild(a);applyCmsVisibility(a,x)}for(const p of (c.customPages||[]).filter(v=>cmsLive(v)&&v.showInMenu!==false).sort((a,b)=>(a.order??999)-(b.order??999))){if(nav.querySelector(`[data-cms-page-link="${p.id}"]`))continue;const a=document.createElement('a');a.dataset.cmsPageLink=p.id;a.href=`page.html?slug=${encodeURIComponent(p.slug)}`;a.textContent=p.menuLabel||p.title||p.label||'Page';a.style.order=String(p.order??999);nav.appendChild(a);applyCmsVisibility(a,p)}}
    for(const x of (c.customButtons||[]).filter(v=>cmsLive(v)&&v.placement&&v.placement!=='nav').sort((a,b)=>(a.order??999)-(b.order??999))){if(document.querySelector(`[data-cms-site-btn="${x.id}"]`))continue;const host=x.placement==='hero'?document.querySelector('.yt-user-home-actions'):x.placement==='footer'?document.querySelector('.yt-user-public-footer'):x.placement==='custom'&&x.host?document.querySelector(x.host):null;if(!host)continue;const a=document.createElement('a');a.dataset.cmsSiteBtn=x.id;a.href=x.href||'#';a.textContent=x.label||'New';a.className=x.placement==='hero'?'yt-user-outline-btn':'yt-cms-footer-link';if(x.newTab){a.target='_blank';a.rel='noopener noreferrer'}host.appendChild(a);applyCmsVisibility(a,x)}
    if(currentPage()==='dashboard.html'){const nav=document.querySelector('.yt-demo-user-nav');if(nav){for(const x of (c.customSidebar||[]).filter(cmsLive).sort((a,b)=>(a.order??999)-(b.order??999))){if(nav.querySelector(`[data-cms-user-sidebar="${x.id}"]`))continue;let el;if(x.targetMode==='link'&&x.href){el=document.createElement('a');el.href=x.href;el.target=x.target||'_self';if(el.target==='_blank')el.rel='noopener noreferrer'}else{el=document.createElement('button');el.type='button';el.dataset.userView=x.targetView||'dashboard';el.addEventListener('click',()=>{if(typeof window.openUserView==='function')window.openUserView(x.targetView||'dashboard')})}el.dataset.cmsUserSidebar=x.id;el.style.order=String(x.order??999);el.innerHTML=`${esc(x.icon||'🔹')} <span>${esc(x.label||'Sidebar Item')}</span>`;nav.appendChild(el);applyCmsVisibility(el,x)}}}
    if(virtualSlug)renderVirtualPage(c,virtualSlug);else{
      for(const [id,v] of Object.entries(c.sections||{})){const sec=document.getElementById(id);if(!sec)continue;sec.hidden=!!(v.hidden||v.inactive||v.deleted||v.status==='draft');if(v.order!==undefined)sec.style.order=String(v.order);const h=sec.querySelector('h2'),p=sec.querySelector('.yt-user-section-title p'),k=sec.querySelector('.yt-user-section-title span');if(h&&v.title)h.textContent=v.title;if(p&&v.text)p.textContent=v.text;if(k&&v.kicker)k.textContent=v.kicker}
      for(const [key,v] of Object.entries(c.cards||{})){const el=document.querySelector(`[data-cms-site-card="${CSS.escape(key)}"]`);if(!el)continue;el.hidden=!!(v.hidden||v.inactive||v.deleted||v.status==='draft');if(v.order!==undefined)el.style.order=String(v.order);const h=el.querySelector('h3'),p=el.querySelector('p'),b=el.querySelector('b');if(h&&v.title)h.textContent=v.title;if(p&&v.text)p.textContent=v.text;if(b&&v.icon)b.textContent=v.icon}
      const main=document.querySelector('.yt-user-public-main');for(const x of (c.customSections||[]).filter(cmsLive).sort((a,b)=>(a.order??999)-(b.order??999))){if(!main||main.querySelector(`[data-cms-section="${x.id}"]`))continue;const sec=document.createElement('section');sec.className='yt-user-public-section';sec.dataset.cmsSection=x.id;sec.style.order=String(x.order??999);sec.innerHTML=`<div class="yt-user-section-title"><span>${esc(x.kicker||'NEW')}</span><h2>${esc(x.title)}</h2><p>${esc(x.content||'')}</p></div>`;main.appendChild(sec);applyCmsVisibility(sec,x)}
    }
  }
