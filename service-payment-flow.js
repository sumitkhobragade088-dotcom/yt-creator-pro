...
function syncUserServiceUI(){
  ...
  userServices.forEach(s=>{if(chosen.has(s.service_name))total+=Number(s.amount||0);});
...
async function loadUserServices(){
  const {data,error}=await supabase.from("service_charges").select("service_name,description,is_active,sort_order,amount").eq("is_active",true).order("sort_order",{ascending:true}).order("service_name",{ascending:true});
  if(error){if($("userServiceRequestMessage"))$("userServiceRequestMessage").textContent=error.message;return;}
  userServices=data||[];
  const sel=$("userServiceType"),box=$("userServiceOptions");if(!sel||!box)return;
  sel.innerHTML=userServices.map(s=>`<option value="${esc(s.service_name)}">${esc(s.service_name)}</option>`).join("");
  box.innerHTML=userServices.length?userServices.map(s=>`<label class="yt-user-service-check"><input type="checkbox" data-user-service value="${esc(s.service_name)}"><span class="svc-main"><span class="svc-name">${esc(s.service_name)}</span>${s.description?`<span class="svc-desc">${esc(s.description)}</span>`:""}</span><span class="svc-price">${money(s.amount)}</span></label>`).join(""):"<div class=\"yt-service-empty\">No active services available.</div>";
  box.querySelectorAll('input[data-user-service]').forEach(i=>i.addEventListener("change",()=>{const o=[...sel.options].find(x=>x.value===i.value);if(o)o.selected=i.checked;syncUserServiceUI();}));
  syncUserServiceUI();
}
async function openPayU(paymentId, button){
  ...
  try{
    ...
    const form=document.createElement("form");form.method="POST";form.action=data.endpoint;form.style.display="none";
  ...
  }catch(e){alert(e?.message || "PayU payment start nahi hua.");if(button){button.disabled=false;button.textContent=old;}}
}
async function waitForPayment(requestId, customerId){
  for(let i=0;i<12;i++){
    const {data,error}=await supabase.from("payments").select("id,status,amount").eq("request_id",requestId).eq("customer_id",customerId).maybeSingle();
    if(!error&&data)return data;await new Promise(r=>setTimeout(r,250));
  }return null;
}
async function submitAndPay(){
  const btn=$("submitPaidServiceRequest")||$("submitUserServiceRequest");
  const msg=$("userServiceRequestMessage");
  const selectedServices=selected();
  if(!selectedServices.length){if(msg)msg.textContent="Please select at least one service.";return;}
  const customer=await currentCustomer();
  if(!customer){if(msg)msg.textContent="Customer profile not found.";return;}
  btn.disabled=true;if(msg)msg.textContent=`Creating request for ${selectedServices.length} service${selectedServices.length>1?"s":""}...`;
  try{
  ...
  }catch(e){if(msg)msg.textContent=e?.message||"Request/payment start failed.";btn.disabled=false;}
}
function paymentActionLabel(status){if(status==="failed")return "Retry Payment";if(status==="cancelled"||status==="canceled")return "Pay Again";return "Pay Now";}
async function loadPaidRequestsAndPayments(){
  const customer=await currentCustomer();if(!customer)return;
  ...
  if(rErr)console.error("Requests:",rErr);if(pErr)console.error("Payments:",pErr);
  ...
  const requestBox=$("paidRequestList");if(requestBox)requestBox.innerHTML=paidRequests.length?paidRequests.map(r=>`<div class="request-row"><div><b>${esc(r.service_type||"Service")}</b><small>${new Date(r.created_at).toLocaleString("en-IN")}</small></div><span class="yt-request-status ${esc(statusKey(r.status))}">${esc(r.status||"pending")}</span></div>`).join(""):"<p>No paid service requests yet.</p>";
  const paymentBox=$("userPaymentsList");if(paymentBox)paymentBox.innerHTML=(payments||[]).length?(payments||[]).map(p=>{const st=statusKey(p.status),canPay=st!=="paid";return `<div class="request-row yt-request-payment-row"><div><b>${esc(p.service_name||"Service")}</b><small>${money(p.amount)} · ${esc(p.txnid||"Transaction not started")}</small></div><div class="yt-payment-actions"><span class="yt-pay-chip ${st==="paid"?"paid":st==="failed"?"failed":"pending"}">${esc(st.toUpperCase())}</span>${canPay?`<button type="button" class="yt-user-red-btn" data-retry-payment="${esc(p.id)}">${paymentActionLabel(st)}</button>`:""}</div></div>`;}).join(""):"<p>No payment records yet.</p>";
